...
class BackgroundVoltageArrayTask(Task):
    def __init__(self, size):
        ...
        self.CreateAIVoltageChan("Dev2/ai0","Voltage",DAQmx_Val_Diff,-10.0,10.0,DAQmx_Val_Volts,None)
        self.CfgSampClkTiming("PFI11", 100000, DAQmx_Val_Rising, DAQmx_Val_ContSamps, self.updatadataevery)
...
class MeasureTask(Task):
    def __init__(self):
        Task.__init__(self)
        # Create data storages:
        self.updatadataevery = 100 # previous is 10, I change it to 100
        self.inputchannelsN = 5
        self.data = np.zeros(self.inputchannelsN*self.updatadataevery)
        self.a = []
        self.CreateAIVoltageChan("Dev2/ai0","Voltage",DAQmx_Val_Diff,-2.0,2.0,DAQmx_Val_Volts,None)
        self.CreateAIVoltageChan("Dev2/ai1","Current",DAQmx_Val_Diff,-10.0,10.0,DAQmx_Val_Volts,None)
        self.CreateAIVoltageChan("Dev2/ai2","LED",DAQmx_Val_Diff,-10.0,10.0,DAQmx_Val_Volts,None)
        self.CreateAIVoltageChan("Dev2/ai3", "Piezo", DAQmx_Val_Diff, -2.0, 2.0, DAQmx_Val_Volts, None)
        self.CreateAIVoltageChan("Dev2/ai4", "QPD-Diff", DAQmx_Val_Diff, -0.5, 0.5, DAQmx_Val_Volts, None)
        self.CfgSampClkTiming("PFI11", 10000.0, DAQmx_Val_Rising, DAQmx_Val_ContSamps, self.updatadataevery)  # use the PFI1 to trigger the DAQ to start the task of AI and AO
        self.AutoRegisterEveryNSamplesEvent(DAQmx_Val_Acquired_Into_Buffer, self.updatadataevery, 0)
        self.AutoRegisterDoneEvent(0)
    def EveryNCallback(self):
        read = int32()
...
class PulseTask(Task):
    def __init__(self):
        ...
        self.on = np.array([1, 0, 0, 0, 0, 0, 0, 0], dtype=np.uint8)
        self.off = np.array([0, 0, 0, 0, 0, 0, 0, 0], dtype=np.uint8)
        self.end = np.array([0, 1, 0, 0, 0, 0, 0, 0], dtype=np.uint8)
        # Enable channel for use:
        self.CreateDOChan("Dev2/port0/line0:7", "", DAQmx_Val_ChanForAllLines)
    def pulse(self, fps):
        # Send two short pulses to the LED:
        sleep(5/fps) # wait for a bit longer, which can be used as a background frames
        ...
        self.WriteDigitalLines(1, 1, 10.0, DAQmx_Val_GroupByChannel, self.on, None, None)
        sleep(2/fps)
        self.WriteDigitalLines(1, 1, 10.0, DAQmx_Val_GroupByChannel, self.end, None, None)  #Trig CHI

        #DAQmxWriteDigitalLines(TaskHandle, taskHandle, int32,  numSampsPerChan, bool32, autoStart, float64     timeout, bool32
        # dataLayout, uInt8, writeArray[], int32 * sampsPerChanWritten, bool32 * reserved);
...
class TriangleWaveGenerator(Task):
    ...
    def __init__(self, freq, ampl, off):
        ...
        # Setup DAQ:
        read = int32()
        self.CreateAOVoltageChan("Dev2/ao0","Potential",-10.0,10.0,DAQmx_Val_Volts,None)
...
class SquareWave4CamFPSCtrl(Task):
    """ Makes a custom square wave for comtrol the Frame arte of  the camera
    """
    def __init__(self, freq = 10):
        Task.__init__(self)
        # Wavegenerator properties:
        self.sampleRate = 2000 * freq  # Samples/second
        self.maxNsamples = int(2000 * freq)
        self.ampl = 2
        self.offset = 2
        # Check wave for invalid values:
        if abs(self.ampl) + abs(self.offset) > 10.0:
            print("Tried to supply a potential over (-)10 V")
            return
        # Make array of wave:
        self.Nsamples = int(self.sampleRate)
        self.t = np.linspace(0, 1, self.Nsamples, endpoint=False)
        self.SquWavedata = np.sign(np.sin(2 * np.pi * freq * self.t))* self.ampl + self.offset
        # Setup DAQ:
        read = int32()
        self.CreateAOVoltageChan("Dev2/ao1", "SquareWave", -5.0, 5.0, DAQmx_Val_Volts, None)
        self.CfgSampClkTiming("", self.sampleRate, DAQmx_Val_Rising, DAQmx_Val_ContSamps, self.maxNsamples)
        self.WriteAnalogF64(self.Nsamples, 0, 10.0, DAQmx_Val_GroupByScanNumber, self.SquWavedata, byref(read), None)
