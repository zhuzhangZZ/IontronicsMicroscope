...
from log import get_logger, log_to_file, log_to_screen # load the log function

import time
import sys,os
currentcwd = os.getcwd()
currentcwd = "C:\ZhuZhang\Iontronics4BSI\IontronicsMicroscope\Instrumentation\IMGUI"
photometricsDriverPath = os.path.join(currentcwd, "UUTrack\Controller\devices\photometrics\src") ## the src of the PyVCAM, and I install it the develoing mode with "pip install -e ."
sys.path.append(photometricsDriverPath)
from pyvcam import pvc
from pyvcam.camera import Camera

from _skeleton import cameraBase # load the skeleton base of the Camaer
ureg = UnitRegistry()
Q_ = ureg.Quantity
